...
def run_backtest(config_path: str, cases_path: str, output_dir: str) -> dict:
    ...
    for case in cases:
        ...
        heats = [assess_profit_protection(item, risk.score).score for item in [btc, *available]]
        ...
        for symbol in case["focus"]:
            ...
            assets.append({"symbol": symbol, "price": round(item.price, 6), "signal": item.signal,
                           ...
                           "max_drawdown_30d": dd30, "grade": _grade(readiness.status, r30, r90, dd30)})
        results.append({"date": str(case["date"]), "name": case["name"], "rationale": case["rationale"],
                        "risk_on": score, "exit_risk": risk.score, "market_regime": positioning.market_regime,
    ...
    decisive = [asset for case in results for asset in case["assets"] if asset["grade"] in {"GOOD", "BAD", "MISSED UPSIDE"}]
    good = sum(asset["grade"] == "GOOD" for asset in decisive)
    summary = {"cases": len(results), "decisive_asset_calls": len(decisive), "good": good,
               "bad": sum(asset["grade"] == "BAD" for asset in decisive),
               "missed_upside": sum(asset["grade"] == "MISSED UPSIDE" for asset in decisive),
    ...
    for case in results:
        for asset in case["assets"]:
            rows.append(f"<tr><td>{escape(case['date'])}<br><small>{escape(case['name'])}</small></td><td><b>{escape(asset['symbol'])}</b></td>"
                        ...
                        f"<td>{asset['max_drawdown_30d']:+.1f}%</td><td><b>{escape(asset['grade'])}</b></td></tr>")
...
<p class='muted'>{escape(payload['limitations'])}</p><div class='scroll'><table><tr><th>Date</th><th>Asset</th><th>Market readiness</th><th>Asset readiness</th><th>Signal</th><th>+7D</th><th>+30D</th><th>+90D</th><th>30D max DD</th><th>Grade</th></tr>{''.join(rows)}</table></div></body></html>"""
