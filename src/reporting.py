...
def render(score: float, previous: float | None, coins: list[CoinResult], notes: list[str], context: dict, exit_risk: ExitRiskResult, history: list[float], dominance: dict | None = None) -> tuple[str, str]:
    ...
    rows = "".join(f"<tr><td><b>{escape(c.symbol)}</b></td><td>{c.score:.0f}</td><td><b>{c.signal}</b></td><td>${_n(c.price, 4)}</td><td>{c.rel_7d:+.1f}%</td><td>{c.rel_30d:+.1f}%</td><td>{c.rsi:.1f}</td><td>{c.volume_ratio:.1f}x</td><td>{_n(c.entry_low,4)}–{_n(c.entry_high,4)}</td><td>{_n(c.target1,4)} / {_n(c.target2,4)}</td><td>{_n(c.invalidation,4)}</td></tr>" for c in top)
