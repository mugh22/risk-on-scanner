...
def _bars(components: dict[str, float]) -> str:
    return "".join(f"<tr><td style='width:185px'>{escape(name)}</td><td style='font-family:monospace;letter-spacing:1px;color:{_color(value)}'><b>{'█' * round(value / 10)}{'░' * (10 - round(value / 10))}</b></td><td style='width:36px;text-align:right'><b>{value:.0f}</b></td></tr>" for name, value in components.items())
...
def _history_chart(history: list[float]) -> str:
    ...
    blocks = "▁▂▃▄▅▆▇█"
    spark = "".join(blocks[min(7, int(value / 12.5))] for value in history[-20:])
    values = " → ".join(f"{value:.0f}" for value in history[-6:])
    return f"<div style='font-family:monospace;font-size:25px;letter-spacing:3px;color:{_color(history[-1])}'><b>{spark}</b></div><div class='muted'>Recent scores: {values} &nbsp;|&nbsp; Latest: <b>{history[-1]:.0f}</b></div>"
